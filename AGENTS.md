...
## Stack and checks
React, TypeScript and Vite. The owner selected direct-scroll A on20September2026 and authorized completing all four scenes, lifting the historical5MB transfer ceiling for quality. This supersedes the earlier three-second gesture-lock interaction. Three original-source high-quality movies track native vertical document scrolling; retain reading holds, immediate reversal, actual-frame caption timing and later-film preparation near the viewport. Preserve DESIGN.md, approved artwork/copy and the original logo. No new generation: the owner asks to conserve Higgsfield credits. Record actual transfer and device limitations honestly. Package versions in package.json are authoritative.
- `npm test`: investor behavior tests.
- `npm run typecheck`: TypeScript.
- `npm run lint`: ESLint.
- `npm run build`: production bundle plus pre-rendered essential HTML.
- `npm run test:e2e`: real Chrome and WebKit scroll/codec/responsive regressions; expects a server on 127.0.0.1:5184. Install its WebKit binary once with `npx playwright install webkit`.
