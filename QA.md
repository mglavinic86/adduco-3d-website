...
Public Sites version25 is deployed from `f211dcc016fba376cf3d91fc89cedec6955abcc9`; all18 finishing browser checks pass on that URL. The follow-up CI-only correction measures the first WebKit-policy request rather than racing a valid HEVC→H.264 retry, and tests Space/PageDown in separate fresh browser contexts so their native scroll animations cannot overlap. There are no additional app/media changes; the version24 performance samples below remain applicable to that runtime apart from accessible labels.
...
### Compositor bootstrap correction

WebKit could combine an initial loadeddata seek with its first painted frame, so the requested first frame never received its presentation callback. Wait for that initial compositor callback, then service the retained scroll target on the next animation frame. This does not start an idle loop, replay the film or delay native page scrolling. The focused rerun passes all6applicable cold cases (2CDP-only skips): WebKit first-requested frame133ms portrait/61ms landscape unthrottled, subsequent requests33–34ms. Chrome4G first451/458ms, arbitrary cold requests152–918ms. Metadata/currentTime alone is never used as proof of presentation. Failed and successful JSON samples are retained in the external evidence archive.
