...
- Opening image is preloaded before deferred JavaScript with orientation-specific picture selection and inline blurred fallback. The first film prepares when the stage is visible; direct business-section visits request no films. IntersectionObserver prepares later films near their transitions. Native video elements retain buffers for reversal. A direct jump prepares its requested movie immediately and exposes its caption/still without waiting for playback.
...
- src/scenes.ts holds approved Croatian captions. BusinessContent renders the sourced records from content/projects.json; ProjectRecords accepts real photographs only, with an honest pending state while images are empty. Null year/client values are unknown, not invented. InquiryForm prepares a reviewable mailto draft with a clipboard button and graceful clipboard denial; no false delivery confirmation or new service.
...
Sites configuration is .openai/hosting.json. Preserve the existing public audience and noindex metadata; publish the exact verified source/build through Sites and sync the public GitHub repository. GitHub Actions performs source checks and Chrome browser regressions with one worker, but does not deploy. Webhook delivery, confirmed legal/privacy data and search indexing remain pending owner inputs. No inquiry is claimed sent without a delivery integration.

## Adding approved photography

Edit content/projects.json. Keep the confirmed title, location, role, client, sourceUrl and source description. The optional year is the project year, never inferred from a report date. Add approved local files under public/assets/projects/ and image entries containing src, alt and caption. Empty images render “Fotografije u pripremi.” Do not use cinematic artwork as portfolio photography.

The finishing pass uses an 820svh track (previously 1020svh), preserves all source frames and reading holds, and restores a fully readable caption 120ms after camera movement stops, with a 160ms opacity transition. This finite caption timer never animates the camera.
